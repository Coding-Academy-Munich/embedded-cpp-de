...
// %%
int getMeasurement(int a, int b) { return a + b; }

// %%
int computeDataForNextTimestep(int measurement) { return measurement + 1; }

// %%
bool isValidResult(int result) { return result > 0; }
...
// %%
void saveResult(int newResult, std::vector<int>& results)
{
    results.push_back(newResult);
}
...
// %%
int performMeasurementAndProcessResult(int a, int b, std::vector<int>& results)
{
