...
// %%
class SMSAdapter : public MessagingService {
private:
    SMS sms;

public:
    SMSAdapter(SMS sms) : sms(sms) {}

    void Send(const User& to, std::string message) override {
        sms.SendText(to.phoneNumber, message);
    }

    void Receive(const User& from) override { sms.ReceiveText(from.phoneNumber); }
};

// %%
class EmailAdapter : public MessagingService {
private:
    Email email;

public:
    EmailAdapter(Email email) : email(email) {}

    void Send(const User& to, std::string message) override {
        email.SendEmail(to.emailAddress, "Mail from Chat App", message);
    }

    void Receive(const User& from) override { email.ReceiveEmail(from.emailAddress); }
};

// %%
class InAppChatAdapter : public MessagingService {
private:
    InAppChat chat;

public:
    InAppChatAdapter(InAppChat chat) : chat(chat) {}

    void Send(const User& to, std::string message) override {
        chat.SendMessage(to, message);
    }

    void Receive(const User& from) override { chat.ReceiveMessage(from); }
};
...
// %%
std::unique_ptr<MessagingService> smsAdapter{std::make_unique<SMSAdapter>(sms)};
std::unique_ptr<MessagingService> emailAdapter{std::make_unique<EmailAdapter>(email)};
std::unique_ptr<MessagingService> chatAdapter{std::make_unique<InAppChatAdapter>(chat)};
...
// %%
messagingServices.push_back(std::move(smsAdapter));
messagingServices.push_back(std::move(emailAdapter));
messagingServices.push_back(std::move(chatAdapter));
